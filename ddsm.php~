...
Based on Next of Kin plugin Tzafrir Rehan  (email : tzafrir@tzafrir.net)
...
load_plugin_textdomain('DDSM','wp-content/plugins/DDSM');
update_option('tz_DDSM_ver', '1.1');
function tz_DDSM_activate() { //Create option in database
global $current_user;
//Last Login
add_option('tz_DDSM_ll_'.$current_user->user_login, time(), $current_user->user_login.'\'s last login time', 'yes');
//Last Iteration
add_option('tz_DDSM_lastiteration', (time() - 7200));
}

function tz_DDSM_init() { //Runs every load of wordpress
global $current_user;	//but only if
if (get_option('tz_DDSM_active_'.$current_user->user_login) == true 
&& (time() - get_option('tz_DDSM_ll_'.$current_user->user_login)) > 43200) {//the plugin is activated in own menu per current user (not just plugins menu)
...
if ((time() - get_option('tz_DDSM_lastiteration')) > 7200) { // The following is a bit heavy, so we'll only do it once every two hours
...
if ((time() - get_option('tz_DDSM_ll_'.$user)) > ($cur_options['interval1'] * $weeks) 
&& get_option('tz_DDSM_step1_'.$user) != true) { // After interval 1
...
if ((time() - get_option('tz_DDSM_ll_'.$user)) > (($cur_options['interval1'] + $cur_options['interval2']) * $weeks)
&& get_option('tz_DDSM_step2_'.$user) != true) { // After interval 2
...
if ((time() - get_option('tz_DDSM_ll_'.$user)) > (($cur_options['interval1'] + $cur_options['interval2'] + $cur_options['interval3']) * $weeks)
&& get_option('tz_DDSM_step3_'.$user) != true) { // After interval 3 (Death)
...
function tz_DDSM_menu() { // function that calls the option menu function
    if (function_exists('add_options_page')) {
        add_options_page(__('Digital Deads Man Switch (DDSM) Options', 'DDSM'), __('Digital Deads Man Switch (DDSM)', 'DDSM'), 8, basename(__FILE__), 'tz_DDSM_menupage');
    }
 }
...
function tz_DDSM_menupage() { //the options menu
global $current_user;
$ddg_ddsm_users = get_option('tz_DDSM_users');// get users list
echo '<div class="wrap"><h2>'.__('Digital Deads Man Switch (DDSM) Options', 'DDSM').'</h2>';
 if (isset($_POST['ddg_ddsm_update']) || isset($_POST['ddg_ddsm_update_mails']) || isset($_POST['ddg_ddsm_reset'])) {
    ?><div class="updated"><p><strong><?php 
_e('Options updated succesfully.', 'DDSM'); 
if ($_POST['ddg_ddsm_act'] == 'on') {
echo '<br />
'.__('Plugin activated - Now go and edit your messages and options below!', 'DDSM'); }
if (isset($_POST['ddg_ddsm_reset'])) {
echo '<br />
'.__('Timestamps and sent mails status reset.', 'DDSM');
}
    ?></strong></p></div><?php 
}
...
if ($_POST['ddg_ddsm_act'] == 'on') { // If plugin was just enabled (through options menu, not activated in plugins page)
...
<p><label><input type="checkbox" name="ddg_ddsm_act"';
if (get_option('tz_DDSM_active_'.$current_user->user_login) == true) { // all options are user specific. will this work in MU?
echo 'checked="checked"';
}
echo ' />'.__('Enable Digital Deads Man Switch (DDSM) plugin functionality', 'DDSM').'</label></p>
';
if (get_option('tz_DDSM_active_'.$current_user->user_login) == true) { // Most of what comes next only loads if plugin is enabled for the logged in user.
...
if ($_POST['ddg_ddsm_email'] == '') {$options['email'] = $current_user->user_email;} //No blank self email allowed - will result in false deaths.
else { 
$options['email'] = strip_tags(stripslashes($_POST['ddg_ddsm_email']));
}
if ($_POST['ddg_ddsm_name'] == '') {$options['name'] = $current_user->display_name;} //No blank name allowed - we want the recepient to know who's sending this
else { 
...
if (isset($_POST['ddg_ddsm_update_mails'])) { // If user hits Update Messages button
...
<input type="hidden" name="tz_more" value="1" />
                                <table class="optiontable">
                                        ...
                                        <tr valign="top">
                                                ...
                                                <td>
                                                        <input style="direction:ltr;" type="text" name="ddg_ddsm_email" value="' . $options['email'] . '" size="50"/>
                                                </td>
                                        </tr>
<tr valign="top">
                                                <th scope="row">'.__('Email(s) to receive your electronic will: ', 'DDSM').' '.__('(seperate multiple addresses with commas)', 'DDSM').'</th>
                                                <td>
                                                        <input style="direction:ltr;" type="text" name="ddg_ddsm_email_other" value="' . $options['email_other'] . '" size="50"/>
                                                </td>
                                        </tr>
<tr valign="top">
                                                <th scope="row">'.__('Your name: (will appear as sender)', 'DDSM').'</th>
                                                <td>
                                                        <input type="text" name="ddg_ddsm_name" value="' . $options['name'] . '" size="50"/>
                                                </td>
                                        </tr>
                                        <tr valign="top">
                                                <th scope="row">'.__('Interval 1: (time between last login into the system and first warning mail)', 'DDSM').'</th>
                                                <td>
<select name="ddg_ddsm_int1" size="1">';
$tz_int = 1;
while ($tz_int < 11) {
...
echo '</select> '.__('Weeks', 'DDSM').'
                                        ...
                                        <tr>
                                              <th scope="row">'.__('Interval 2: (time between first warning mail to seconds warning mail)', 'DDSM').'</th>
                                                <td>
                                                        <select name="ddg_ddsm_int2" size="1">';
$tz_int = 1;
while ($tz_int < 11) {
...
<tr>
                                                ...
                                                <td>
                                                        <select name="ddg_ddsm_int3" size="1">';
$tz_int = 1;
while ($tz_int < 11) {
...
?>
<div class="submit">
  <input type="submit" name="ddg_ddsm_update" value="<?php
    _e('Update Options', 'DDSM')
?> &raquo;" /></div>
  </form></div><?php
...
<h3>'.sprintf(__('Message 1: <small>(will be sent to you after %s weeks of inactivity)</small>', 'DDSM'), $options['interval1']).'</h3><br />
<input type="text" name="ddg_ddsm_subj1" value="' . $optionsm['subject1'] . '" size="50"/>
<textarea style="width: 98%; font-size: 12px;" rows="6" cols="60" name="ddg_ddsm_msg1" >'.$optionsm['email1'].'</textarea>
<h3>'.sprintf(__('Message 2: <small>(will be sent to you after %s more weeks of inactivity)</small>', 'DDSM'), $options['interval2']).'</h3><br />
<input type="text" name="ddg_ddsm_subj2" value="' . $optionsm['subject2'] . '" size="50"/>
<textarea style="width: 98%; font-size: 12px;" rows="6" cols="60" name="ddg_ddsm_msg2" >'.$optionsm['email2'].'</textarea>
<h3>'.__('Message 3: <small>(will be sent to the email address(es) you chose to receive your details, at the same time as message 2)</small>', 'DDSM').'</h3><br />
<input type="text" name="ddg_ddsm_subj3" value="' . $optionsm['subject3'] . '" size="50"/>
<textarea style="width: 98%; font-size: 12px;" rows="6" cols="60" name="ddg_ddsm_msg3" >'.$optionsm['email3'].'</textarea>
<h3>'.sprintf(__('Message 4: <small>(will be sent to your chosen email address(es) after %s more weeks of inactivity)</small>', 'DDSM'), $options['interval3']).'</h3><br />
<input type="text" name="ddg_ddsm_subj4" value="' . $optionsm['subject4'] . '" size="50"/>
<textarea style="width: 98%; font-size: 12px;" rows="6" cols="60" name="ddg_ddsm_msg4" >'.$optionsm['email4'].'</textarea>
<br />
<div class="submit">
  <input type="submit" name="ddg_ddsm_update_mails" value="'.__('Update Messages', 'DDSM').' &raquo;" /></div>
  </form></div>
  
  <div class="wrap">
  <form method="post">
  <div class="submit">
  <p>'.__('This button will reset last login timestamps and sent mail status. Use it in case you got the first warning email.', 'DDSM').'</p>
  <input type="submit" name="ddg_ddsm_reset" value="'.__('Reset', 'DDSM').' &raquo;" /></div>
  </form></div>
  ';
}
 ...
 add_action('admin_menu', 'tz_DDSM_menu');
 add_action('activate_DDSM/DDSM.php', 'tz_DDSM_activate');
 add_action('init', 'tz_DDSM_init');
